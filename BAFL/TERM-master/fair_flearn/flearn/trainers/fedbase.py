...
class BaseFedarated(object):
    ...
    def setup_clients(self, dataset, data_partition_seed, model=None):
        ...
        all_clients = [Client(u, g, train_data[u], test_data[u], data_partition_seed, model) for u, g in
                       zip(users, groups)]
        return all_clients
    ...
    def select_clients(self, round, pk, num_clients=20):
        ...
        if self.sampling == 1:
            ...
            if end_index > begin_index:
                ...
                indices = list(range(begin_index, len(self.clients))) + list(
                    range(0, end_index))  # python3, must convert to list first, then add
            return indices, np.asarray(self.clients)[indices]
        ...
        # oracle
            ...
            for i in range(num_clients):
                tmp = np.random.choice(
                    range(int(i * (len(self.clients) / num_clients)), int((i + 1) * (len(self.clients) / num_clients))),
                    1, replace=False)[0]
                indices.append(tmp)
    ...
    def aggregate(self, wsolns):
        ...
        base = [0] * len(wsolns[0])  # how many layers
        for soln in wsolns:  # w is the number of samples
            for i, v in enumerate(soln):
                base[i] += v.astype(np.float64)
        averaged_soln = [v / num_clients for v in base]
